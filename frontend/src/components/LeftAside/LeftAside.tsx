...
const LeftAside = () => {
  const {
    ...
    loadingState,
    errors,
  } = useAppSelector(selectTagsSelector);
  ...
  const renderUsers = useMemo(() => {
    const mappedUsers = tagsItems.users?.map(
      ({ avatarUrl, fullname, username }) => (
        <div
          key={username}
          className="hover:bg-gray-200 p-2 cursor-pointer flex items-center">
          ...
          <div className="flex justify-around w-full items-center">
            ...
            <button className="btn-secondary bg-gray-800 text-white justify-center text-xs leading-0 h-6 w-16 p-0">
              Follow
            </button>
          </div>
        </div>
